    ...
    <div className="min-h-screen flex flex-col bg-[#020617] text-slate-200">
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500 font-mono mt-1 uppercase tracking-[0.2em]">Enterprise Multimodal Suite</p>
            </div>
          </div>
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full p-6 grid grid-cols-12 gap-6 overflow-hidden">
          ...
          <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-5 space-y-6 shadow-xl backdrop-blur-sm">
            ...
            >
              ...
              <div className="text-center">
                ...
                <p className="text-[10px] text-slate-600 mt-1 uppercase font-mono">PDF / IMG / VID / AUD</p>
              </div>
            </div>
            ...
            {mode === 'CHAT' && (
              <div className="space-y-4">
                 ...
                 <div className="bg-slate-950/60 p-4 rounded-xl border border-slate-800 text-[9px] text-slate-500 text-center leading-relaxed font-mono">
                   Conversational Low-Latency Engine. Real-time audio modulation via native modality.
                 </div>
              </div>
            )}
            ...
            {mode === 'TRANSCRIPTION' && (
              <div className="space-y-4">
                ...
                <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 text-[9px] text-slate-500 text-center uppercase tracking-widest">
                  FLASH LITE ENCODING READY
                </div>
              </div>
            )}
        ...
        <section className="col-span-12 lg:col-span-9 flex flex-col gap-6 overflow-hidden">
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 gap-3">
                               {state.result.groundingUrls.map((g: any, i: number) => (
                                 <a key={i} href={g.web?.uri || g.maps?.uri} target="_blank" className="bg-slate-900/50 border border-slate-800 p-4 rounded-xl flex items-center gap-4 hover:border-cyan-500/50 transition-all group shadow-md">
                                   <div className="h-10 w-10 bg-slate-950 rounded-xl flex items-center justify-center text-slate-600 group-hover:text-cyan-400 transition-colors shadow-lg">
                                      {g.web ? (
                                        <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" /></svg>
                                      ) : (
                                   ...
                                   <div className="flex-1 min-w-0">
                                     <span className="text-[10px] font-bold text-slate-400 truncate block group-hover:text-slate-200 uppercase tracking-widest">{g.web?.title || g.maps?.title || 'External Protocol'}</span>
                                     <span className="text-[9px] text-slate-600 truncate block font-mono mt-1 opacity-60">{g.web?.uri || g.maps?.uri}</span>
                                   </div>
                                 </a>
                               ))}
                            </div>
          ...
          {/* Bottom Pane: Structure & Data Tabs / Hugging Face results */}
                ...
                {mode === 'HUGGING_FACE' ? (
                  ...
                  <div className="animate-in slide-in-from-bottom-2 duration-700 h-full">
                     {activeResultTab === 'sections' && (
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-4">
                         {state.result.sections.map((s, idx) => (
                           <div key={idx} className="bg-slate-900/60 border border-slate-800 p-6 rounded-2xl space-y-4 hover:border-slate-600 hover:bg-slate-900/80 transition-all group shadow-lg">
                             <div className="flex justify-between items-center">
                               <h4 className="text-xs font-black text-cyan-400 uppercase tracking-wider">{s.title}</h4>
                               <div className="px-2.5 py-1 rounded-full border border-slate-800 bg-slate-950 text-[9px] font-black font-mono text-slate-600 shadow-inner">RISK: {(s.riskScore * 100).toFixed(0)}%</div>
                             </div>
                             <p className="text-[11px] text-slate-400 leading-relaxed font-medium group-hover:text-slate-300 transition-colors">{s.content}</p>
                             <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-800/50">
                                {s.entities?.map((e: any, ei: number) => (
                                  <span key={ei} className="px-2.5 py-1 bg-slate-950/80 border border-slate-800 rounded-lg text-[8px] font-black text-slate-500 uppercase tracking-tighter hover:text-cyan-500 transition-colors shadow-sm">
                                    {e.value} <span className="opacity-40 font-mono ml-2">[{e.type}]</span>
                                  </span>
                                ))}
                             </div>
                           </div>
                     ...
                     {activeResultTab === 'markdown' && (
                       <div className="bg-slate-950/80 rounded-2xl p-8 border border-slate-800 min-h-full shadow-inner selection:bg-cyan-500/30">
                          <pre className="text-xs font-mono text-slate-500 whitespace-pre-wrap leading-loose tracking-wide">{state.result.markdown}</pre>
                       </div>
                     )}
                     {activeResultTab === 'json' && (
                       <div className="bg-slate-950/80 rounded-2xl p-8 border border-slate-800 min-h-full shadow-inner overflow-auto">
                          <pre className="text-xs font-mono text-cyan-800/80 leading-relaxed">{JSON.stringify(state.result.jsonSchema, null, 2)}</pre>
                       </div>
                     )}
                     {activeResultTab === 'web' && (
                       <div className="h-full flex flex-col gap-6">
                          <div className="flex justify-between items-center bg-slate-900/60 p-4 rounded-2xl border border-slate-800 shadow-xl">
                             <div className="flex flex-col">
                                <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Prototype schematic</span>
                                <span className="text-[8px] text-slate-700 font-mono mt-1 uppercase">Ready for deployment</span>
                             </div>
                             <button onClick={() => navigator.clipboard.writeText(state.result?.websiteCode || '')} className="px-5 py-2.5 bg-cyan-600/10 border border-cyan-500/30 rounded-xl text-[10px] font-black text-cyan-400 hover:bg-cyan-500 hover:text-white transition-all shadow-lg">Copy Prototype</button>
                          </div>
                          <div className="flex-1 bg-white rounded-2xl shadow-2xl border border-slate-800 overflow-hidden group">
                             {state.result.websiteCode ? (
                               <iframe srcDoc={state.result.websiteCode} className="w-full h-full" title="Synthesized Web UI" />
                             ) : (
                               <div className="h-full flex flex-col items-center justify-center gap-4 bg-slate-950 text-slate-800">
                                  <svg className="h-12 w-12 opacity-10" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" /></svg>
                                  <p className="text-[10px] font-black uppercase tracking-widest opacity-20">No UI modulation found</p>
                               </div>
                             )}
                          </div>
                       </div>
                  ...
                  <div className="h-full flex flex-col items-center justify-center opacity-5 gap-8">
                    ...
                    <p className="text-[12px] font-black uppercase tracking-[0.4em] text-center max-w-sm leading-relaxed">Structural schematic matrix awaiting synthesis stream</p>
                  </div>
                )}
             </div>
          </div>
      ...
      <footer className="h-10 border-t border-slate-800 bg-slate-950 px-8 flex items-center justify-between text-[9px] font-mono text-slate-600 tracking-widest uppercase font-black">
         <div className="flex items-center gap-8">
           <div className="flex items-center gap-2.5">
             <div className={`h-1.5 w-1.5 rounded-full ${state.step === ProcessingStep.FAILED ? 'bg-red-500 animate-pulse' : 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.4)]'}`} /> 
             <span>Protocol: Status Ready</span>
           </div>
           <div className="hidden sm:flex items-center gap-2.5">
             <div className="h-1.5 w-1.5 rounded-full bg-cyan-500 shadow-[0_0_8px_rgba(34,211,238,0.4)]" /> 
             <span>Encryption: AES-256</span>
           </div>
         </div>
         <div className="hidden lg:flex gap-8 items-center opacity-40">
           <div className="flex items-center gap-2"><span>Latency:</span> <span className="text-slate-400">14ms</span></div>
           <div className="flex items-center gap-2"><span>Engine:</span> <span className="text-slate-400">v5.2 Platinum</span></div>
           <span className="text-slate-700 opacity-20">|</span>
           <span className="tracking-widest">DocuMind AI © 2025</span>
         </div>
      </footer>
    </div>
  );
